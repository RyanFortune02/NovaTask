...
const ClassTimeTracker = ({todos}) => {
    ...
    const deleteTimeEntry = async (todoId) => {
        try {
            const response = await fetch(`http://localhost:8000/api/times/${todoId}/`, {
                method: 'DELETE',
            });
    ...
    const formatTime = (totalMinutes) => {
        ...
        return `${hours > 0 ? `${hours}h ` : ''}${minutes > 0 ? `${minutes}m` : ''}`; // Return formatted time
    };
